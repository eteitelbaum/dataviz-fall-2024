...
**Submission note**: For this assignment you should submit your project folder to Blackboard as with previous assignments.

## Step 1: Wrangle your data (20 pts)

Load `vdemlite`, `dplyr` and `readr` into a separate R script or code chunk in a Quarto document. Select `country_name`, `year`, and three V-Dem democracy indicators: `v2x_polyarchy`, `v2x_libdem`, and `v2x_partipdem`. Now use `searchdem()` to select four additional indicators related to democracy that you would like to explore. Next use `fetchdem()` to download the data. Have a `glimpse()` at the data to make sure everything is there. Then store your data in an object called `vdem_trends` and then save it as a CSV file by the same name. 
...
Now you need to render the plot. In the `ggplot()` call your data is going to be the saved in the second reactive function `plot_df()` (don't forget the parentheses!). In `aes()` `x` will simply be `year` but y is a little tricky. Here you need to call `.data[[input$indicator]]` You might have guessed the `input$indicator` part, but to [non-standard evaluation](http://adv-r.had.co.nz/Computing-on-the-language.html) in `aes()`, you cannot pass your column into ggplot before you remove the parentheses. See [Lecture 7.2](https://dataviz-gwu.rocks/slides/week-7.2#/tidy-evaluation-2) for more on this.
